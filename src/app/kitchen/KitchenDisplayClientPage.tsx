...
// Mock data updated for simpler statuses and no guest names on orders
...
export default function KitchenDisplayClientPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Potentially add new mock orders or update existing ones to simulate real-time changes
      setLastUpdated(new Date());
    }, 30000); 
  ...
  const handleUpdateStatus = (orderId: string, newStatus: OrderStatus) => {
    if (newStatus === 'Completed') { // Only action is to mark as completed
